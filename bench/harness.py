...
class Record:
    ...
    #: **이 측정이 학습 힌트를 실제로 받았나.** `mode="warm"` 이라고 다 받는 것이 아니다 —
    #: 학습된 질의에서만 서빙되므로, 구별 없이 cold↔warm 을 비교하면 학습이 안 닿은
    #: 그룹의 잡음이 중앙값을 지배한다(실측: G04 는 -3% 인데 G01·G09 가 +81%·+63% 라
    #: 전체가 +63% 로 나와 "학습 기여" 로 읽혔다). `report.mode_split` 이 이것으로 거른다.
    hints: int = 0

    #: **격리가 샜으면 그 이름.** 빈 문자열이면 깨끗하다. 케이스의 정의가 곧 도구
    #: 집합이라, 그 선을 넘은 세션은 **다른 것을 잰 것**이다. 이 필드가 없던 동안
    #: `Record` 가 dataclass 라 `**r` 로 넘긴 값을 **조용히 버렸다** — 검사를 만들어
    #: 놓고도 결과가 전부 `None` 이었다(실측 2026-08-28).
    leak: str = ""
