...
def _cmd_sync(args) -> int:
    ...
    if rep.delete_skipped:
        # **조용하면 `--full` 이 삭제를 봤다고 믿게 된다.** 실제로는 그 스페이스만
        # 아무것도 안 본 상태이고, 지워졌어야 할 페이지가 계속 남아 있다.
        print(f"\n주의: {', '.join(rep.delete_skipped)} 스페이스가 **빈 목록**을 반환해 "
              f"삭제 판정을 건너뛰었습니다.\n"
              f"  볼트에는 그 스페이스의 페이지가 있는데 Confluence 는 0건이라고 답했습니다 — "
              f"스페이스 키가 바뀌었거나 일시적 오류입니다.\n"
              f"  그대로 지웠다면 원본(mirror/raw/)까지 사라져 재싱크 말고는 복구가 없었습니다. "
              f"키를 확인하고 다시 돌리세요.")
    if rep.failed:
