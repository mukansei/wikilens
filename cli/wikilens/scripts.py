...
#: 문자 집합 필터의 기본 문턱. **정본은 여기 하나다** — 근거(위 분포의 골)가
#: 이 파일에 있으므로 값도 같이 둔다. 전에는 `build.py` 와 `credentials.py` 두 곳에
#: `0.10` 이 따로 적혀 있었고, 독스트링 예시는 옛 값(`0.15`)인 채로 남아 있었다.
...
#: `U+0100-017F`. **대소문자를 가리지 않는다** — 이름은 `.lower()` 로 정규화하는데
#: 범위만 대문자를 요구하면, `u+0100-017f` 를 쓴 사람이 "알 수 없는 문자 집합" 과
#: 이름 목록을 받는다(범위를 썼는데 이름을 고치라는 안내다).
_RANGE = re.compile(r"^U\+([0-9A-Fa-f]{4,6})-([0-9A-Fa-f]{4,6})$", re.I)
...
def foreign_word_ratio(text: str, ranges: list[tuple[int, int]]) -> float:
    ...
    **비용은 켰을 때만 든다.** 실측(실코퍼스 2,000문서 4.7MB → 13,933건 환산): 7.7초.
    `build` 전체가 28초이므로 27% 가 얹힌다. 범위 개수는 무관했다(6구간과 30구간이
    같은 시간) — 병목이 범위 검사가 아니라 파이썬 문자 순회다. 기본이 꺼짐이라
    안 쓰는 사람은 이 비용을 안 낸다.
