...
**되돌리기가 쌉니다.** `mirror/pages/` 는 그대로 두므로 설정을 바꿔 다시 빌드하면
돌아옵니다 — 재싱크(네트워크)가 아니라 재빌드(순수 로컬)입니다.

**빠진 문서는 본문 grep 에는 남습니다**(로컬판 스킬의 3단계). 서버판은 색인에서
빠져 완전히 사라지므로, 두 판의 도달 범위가 여기서만 갈립니다.
