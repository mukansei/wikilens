...
> **서버판 사용자용입니다.** 서버를 세우는 쪽이라면 저장소의 `server/README.md`
> 입니다(설치본에는 없습니다). 로컬판 사용 안내는 `plugin/local/README.md`.
...
| `ANALYZER=x (설정은 y)` | 색인을 서버 설정과 다른 분석기로 만들었습니다. 검색은 되지만 설정이 반영 안 된 상태 — 운영자에게 재색인 요청 |
...
```bash
docker compose build                      # 이미지가 배포된 곳은 없습니다 — 직접 빌드합니다
