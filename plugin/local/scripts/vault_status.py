...
def other_plugin() -> str:
    ...
    **배타성이 아니라 우선순위다**(`DECISIONS.md` D13) — 서버판이 켜져 있으면 그 MCP
    도구 4개는 스킬 선택과 무관하게 항상 모델에게 보이므로, "로컬판이 이긴다" 는 규칙은
    강제할 수단이 없다. 이 값은 스킬이 서버판에 양보할 근거이자 `claude plugin disable`
    을 안내할 자리다.
