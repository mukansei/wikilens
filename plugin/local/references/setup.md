# 로컬판 설정 절차 (정본)

이 문서가 설정 절차의 **유일한 정본**입니다. `/wikilens-local:setup` 커맨드도 이걸 읽고
수행합니다 — 절차를 다른 곳에 복제하지 마세요.

로컬판은 **각자 자기 볼트를 만듭니다.** 관리자가 만든 볼트를 받는 게 아니라 본인 토큰으로
싱크하므로, 볼 수 없는 문서는 애초에 볼트에 들어오지 않습니다(권한이 설계상 해결됩니다).

> **아무것도 자동으로 실행하지 마세요.** 싱크는 Confluence에 수천 건을 요청하고 수십 MB를
> 쓰고 수 분이 걸립니다. 각 단계마다 사용자에게 보여주고 승낙을 받은 뒤 실행하세요.

전체는 네 단계입니다. 시작 전에 현재 상태를 확인하세요:

```
python3 "${CLAUDE_PLUGIN_ROOT}/scripts/vault_status.py"
```

`CREDS=`, `CLI=`, `STATUS=` 를 보고 **이미 끝난 단계는 건너뛰세요.**

이후 CLI 호출은 **전부 래퍼를 거칩니다.** 래퍼가 자격증명을 싣고, CLI 위치를 찾고,
`--root` 를 볼트 경로로 채웁니다:

```
CLI="${CLAUDE_PLUGIN_ROOT}/scripts/wikilens_cli.sh"
```

---

## 1. 자격증명 고정 — 건너뛰면 다음 세션에 싱크가 죽는다

CLI는 자격증명을 환경변수로만 읽습니다. 그런데 `export` 는 **그 셸에서만 삽니다.**
Claude Code를 재시작하면 사라져서, 검색은 되는데 갱신만 조용히 안 되는 상태가 됩니다.
그래서 `~/.wikilens/env.sh` 에 고정합니다(볼트 경로를 `config.json` 에 두는 것과 같은 이유).

**`CREDS=shell` 이면** — 이미 셸에 있으니 그대로 옮기면 됩니다:

```
python3 "${CLAUDE_PLUGIN_ROOT}/scripts/setup_vault.py" --capture-env
```

값은 환경에서 파일로 곧장 가고 화면에 찍히지 않습니다. 파일은 600으로 만들어집니다.

**`CREDS=partial` 이면** — 파일은 있는데 토큰 자리가 템플릿(`<...>`) 그대로입니다.
아래 템플릿을 다시 출력하지 말고, `~/.wikilens/env.sh` 를 열어 그 자리를 채우라고
안내하세요. **토큰은 사용자만 입력합니다.**

**`CREDS=none` 이면** — 위 명령이 사용자가 직접 실행할 템플릿을 출력합니다.
**그 명령은 사용자가 실행하게 하고, 토큰을 대신 입력하지 마세요.** 필요한 값:

| 방식 | 환경변수 |
|---|---|
| Server/DC PAT (대개 이것) | `CONFLUENCE_URL`, `CONFLUENCE_TOKEN` |
| Cloud API 토큰 | `CONFLUENCE_URL`, `CONFLUENCE_EMAIL`, `CONFLUENCE_TOKEN` |
| 자체 IAM OAuth2 | `IAM_TOKEN_URL`, `IAM_CLIENT_ID`, `IAM_CLIENT_SECRET` |
| 리버스 프록시 | `CONFLUENCE_HEADERS='X-Forwarded-User: me@corp'` |

SSO 환경이어도 대개 PAT가 동작합니다. **PAT를 먼저 시도하세요.**

**인증이 이상하게 실패하면 `CONFLUENCE_PREFIX=""` 가 탈출구다**(빈 문자열이 유효한 값이다).
게이트웨이가 `/wiki/rest/api/space`만 열어두고 나머지는 로그인 페이지로 리다이렉트하면
자동 판별이 `/wiki` 를 잘못 고릅니다. **다만 먼저 의심할 자리는 아닙니다** —
`detect_prefix()` 가 `/user/current` 까지 이중 검증하도록 고쳐진 뒤로는 강제 지정 없이
동작하는 것을 실측했습니다(2026-08-06). 자격증명이 파일에 있는지(`CREDS=file`)를 먼저 보세요.

## 2. 볼트 위치와 CLI — 한 번에 보여주고 승낙받기

볼트 경로를 정하고, 원하면 권한 등록까지 한 번에 합니다:

```
python3 "${CLAUDE_PLUGIN_ROOT}/scripts/setup_vault.py" --vault <경로> --register-permissions
```

기본은 `~/.wikilens/vault`. **이미 볼트가 있으면 그 경로를 주면 됩니다** — 옮기라고 하지 마세요.

`--register-permissions` 는 볼트를 `~/.claude/settings.json` 의 접근 허용 디렉터리에
넣습니다. 볼트가 어느 프로젝트 밖에 있어서, 안 하면 프로젝트마다 읽기 승인을 다시
받아야 합니다. **전역 설정 변경이므로 반드시 승낙을 받고 붙이세요** — 원치 않으면
빼고 실행하면 되고, 그 경우 프로젝트마다 한 번씩 승인하면 됩니다.

CLI는 볼트를 **만드는 데만** 필요합니다(검색은 파일만 읽으므로 의존성이 없습니다).
`CLI=(미설치)` 면 위 명령의 출력에 설치 명령 한 줄이 딸려 나옵니다 — **그것을 그대로
사용자에게 보여주고 승낙을 받아 실행하세요.** 여기 옮겨 적지 마세요(정본은
`setup_vault.py` 의 `install_command()` 이고, 옮겨 적으면 둘이 갈립니다).

CLI 는 `~/.wikilens/venv` 에 설치됩니다. **자리가 정해져 있어서 설치 뒤에 찾는 단계가
없습니다** — 끝나면 `vault_status.py` 가 곧바로 `CLI=`에 그 경로를 냅니다.

`CLI_SOURCE=` 가 비어 있으면 저장소 위치를 모르는 것입니다 — **비공개 git URL을 사용자에게
요청**한 뒤 `--cli-source "git+<URL>#subdirectory=cli"` 로 기록하고 다시 실행하세요.

## 3. 스페이스 고르고 싱크

```
bash "$CLI" doctor
```

배포 형태·인증 방식·접근 가능한 스페이스 목록을 보여줍니다. **여기서 인증이 실패하면
싱크는 어차피 실패하므로 1단계로 돌아가세요.** 성공하면 스페이스 목록을 사용자에게
제시하고 어느 것을 받을지 고르게 하세요 — 필수 인자이고 기본값이 없습니다.

```
bash "$CLI" sync --space <KEY>
```

- 스페이스가 여럿이면 `--space A --space B` 로 반복하세요.
- **`sync` 한 번이 sync + build 를 다 합니다.** 따로 `build`를 부를 필요 없습니다.
- 수 분 걸립니다. 중단돼도 다음 실행이 이어받습니다.

끝나면 값어치를 판정합니다:

```
bash "$CLI" stats
```

출력의 **"제목과 어휘가 안 겹치는 별칭을 가진 페이지"** 비율을 그대로 보고하세요.
이 비율이 낮으면 어휘 격차가 없다는 뜻이고, **그러면 이 도구 전체가 값어치가 없습니다.**
그 판정까지 정직하게 전달하세요 — 숫자를 좋게 포장하지 마세요.

## 4. 정기 갱신을 제안하세요 (여기서 안 하면 아무도 안 합니다)

볼트는 매일 낡습니다. 그런데 사용자가 그 사실을 아는 경로는 **7일이 지난 뒤 검색을
했을 때 스킬이 덧붙이는 한 줄**뿐입니다. 그 전까지는 낡은 답을 최신으로 믿습니다.

싱크가 끝났으면 자동 갱신을 **한 번 물어보세요.** 승낙하면 등록해 주세요:

```
crontab -l 2>/dev/null | grep -q "venv/bin/wikilens" || {
  (crontab -l 2>/dev/null
   echo "0 9 * * 1 $HOME/.wikilens/venv/bin/wikilens sync --space <KEY> --root $HOME/.wikilens/vault") | crontab -
}
```

- **스페이스 키는 3단계에서 사용자가 고른 것을 넣으세요.** 여럿이면 `--space A --space B`.
- **여기서는 래퍼(`$CLI`)를 쓰지 마세요.** 래퍼 경로에 플러그인 버전이 박혀 있어
  (`…/wikilens-local/0.17.5/…`) 다음 업데이트에 cron 이 조용히 깨집니다. CLI 를 직접
  부르면 `--root` 를 대신 줘야 하고, 자격증명은 CLI 가 `~/.wikilens/env.sh` 에서
  읽으므로 그대로 됩니다.
- `$HOME` 도 **펼쳐서** 넣으세요 — cron 의 환경은 로그인 셸이 아닙니다.
- 주 1회(월요일 오전 9시)가 기본입니다. `STALE_DAYS` 가 7이라 이 주기면 stale 경고를
  볼 일이 없습니다. 더 자주 원하면 사용자에게 맞추세요.
- **거절하면 다시 꺼내지 마세요.** 대신 `/wikilens-local:sync` 로 수동 갱신할 수
  있다고 한 번만 알리세요.

macOS 는 cron 이 전체 디스크 접근 권한을 물을 수 있습니다. 등록이 실패하면 그
사실만 알리고 넘어가세요 — 검색은 그대로 됩니다.

---

마무리로 사용자의 **원래 질문으로 돌아가 곧바로 검색해 답하세요.** 설정을 마쳤다는
보고만 하고 멈추지 마세요.
