...
**기본은 꺼짐이고, 값어치는 랭킹이 아니라 오염 제거로 나타난다.** 이 코퍼스에서
켜면 473건(3.4%)이 빠지는데 30질의 랭킹 대조는 상위 10 적중이 16 → 16 으로 안
변했다. 같은 볼트로 G09 에이전트 벤치를 돌리면 **적중이 1/9 → 4/9 다**(12세션
기준 2/12 → 6/12, p=0.19~0.29 로 유의는 아니다). 모델이 6회 고르던 베트남어
번역본이 0회가 되고 그 자리를 정답이 넷 가져갔다. 갈린 것은 순위가 아니라 **모델이
1위에서 답을 확정하고 멈추느냐**다 — 그래서 순위 지표로는 안 보인다.
