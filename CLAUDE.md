...
3. **"유용했다" 판정 개선** — 신호 하나를 더했다(2026-08-06): **서빙한 힌트를 세션이
   ...
   `sessionFailureRate` 로 분리). 실측으로 자기교정 확인: 힌트를 거부하자 같은 질의의
...
- **서버 `grep` 예산 — ripgrep 으로 한 번 밀었다.** JVM 스캔은 13,921건에 2.44초였고
  예산이 3초라 약 17,000건에서 상시 잘림이 될 참이었다. rg 엔진을 붙여 **약 70,000~
  85,000건**으로 밀었다(실측 4~6배). **D12 의 "65배" 는 이 맥락에서 틀린 수치다** —
  그건 셸에서 맨몸으로 잰 값이고, 서버 안에서는 ACL 목록 만드는 고정비를 두 엔진이
  똑같이 낸다. 근거와 대조 방법은 `DECISIONS.md` D20.
