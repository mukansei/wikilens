...
13. **서버 `grep` 이 사용자 정규식을 그대로 실행했음** → `regex=true` 로 `(.+)+@@@@`
   같은 패턴을 보내면 JVM 백트래킹이 폭발해 요청이 **끝나지 않는다**(실측: 20초 넘겨도
   응답 없음, 서버는 살아 있는데 그 스레드가 영구히 묶임). 한 줄만 보면 0.2초라 안
   보이는데 2,383 문서를 훑으며 **누적**되는 것이 위협이라, 재현하려면 픽스처가
   "여러 문서 × 여러 줄"이어야 한다. 두 번째 경로도 있었다 — 백트래킹 깊이가 스택을
   넘기면 `StackOverflowError` 로 **0.02초 만에 HTTP 500** 이라, 시간 예산이 원리적으로
   못 막는다(`(a|aa)+c` + 매치 실패하는 긴 줄).
   **상한으로 증상을 재던 것을 그만두고 엔진을 바꿨다** — `java.util.regex` 대신
   `com.google.re2j`(ripgrep 과 같은 RE2)다. 유한 오토마타라 입력 길이에 선형이고
   재귀를 안 써서 둘 다 원리적으로 없다. 실측: 끝나지 않던 `(.+)+@@@@` 가 1.62초
   전량 스캔(잘림 없음). 줄 길이 상한은 **지웠다** — 그것 때문에 긴 줄 뒤쪽의 일치를
   조용히 놓치고 있었다. 대가는 역참조·전방탐색인데, 조용히 0건이 되지 않게
   `GrepResponse.error` 로 돌려준다. rg 바이너리는 여전히 안 붙인다 — `DECISIONS.md` D12.
14. **`.mcp.json`이 미설정 env 를 리터럴로 주입** → 변수가 없으면 `"${WIKILENS_SERVER}"`
   문자열이 그대로 들어가 프록시 기본값을 덮어쓰고 `unknown url type` 으로 죽는다.
   env 를 넘기지 않고 프록시가 `os.environ` 에서 직접 읽게 한다.
15. **볼트 파일 하나가 깨진 UTF-8 이면 grep 전체가 500** — 기본 디코더가 `REPORT` 라
   **읽는 도중에** `MalformedInputException` 을 던진다. 파일 *열기*만 감싼
   `runCatching` 밖이라 그대로 나가고, 그 한 파일 때문에 나머지 문서 전부를 잃었다
   (`read` 도 같은 이유로 500). 싱크가 디스크 부족이나 중단으로 멀티바이트 문자를
   반토막 내면 실제로 생긴다. `REPLACE` 디코더로 읽어 깨진 지점만 `U+FFFD` 가
   되게 했다 — 건너뛰면 그 문서가 통째로 검색에서 사라진다.
16. **`regex` 토글이 문법이 아니라 대소문자 민감도를 바꿨음** — 리터럴 경로는
   `ignoreCase` 인데 RE2 기본은 구분이라, 같은 문자열이 플래그 하나로 다른 답을 냈다
   (실측: 본문 `Coway` 에 `coway` 가 리터럴 1건 · 정규식 0건). 도구 설명은 이 플래그가
   **문법만** 바꾼다고 말하므로 설명이 거짓이 된 상태였다. `CASE_INSENSITIVE` 로 맞췄고,
   rg 프로세스를 붙인다면 `-i` 를 같이 넘겨야 한다. **엔진을 바꿀 때 딸려오는 기본값이
   조용히 의미를 바꾸는 자리** — 실볼트로는 안 갈리고 단위로만 드러났다.
...
2026-08-06 추가: 기동 적재(`vaultBootstrap`), 샤딩 뒤2/1단계(D11), "유용했다" 판정
(우선순위 3번 완료), **서버 `grep` 정규식 엔진을 RE2 로 교체**(조용히 실패 13·15·16번을
한 번에 없앴다 — D12).
