...
13. **서버 `grep` 이 사용자 정규식을 그대로 실행했음** → `regex=true` 로 `(.+)+@@@@`
   ...
   "여러 문서 × 여러 줄"이어야 한다. 패턴 길이 상한·시간 예산·줄 길이 상한 셋으로
   막았다(4.2초에 `truncated=true` 로 정상 반환). **시간 예산이 못 막는 두 번째 경로가
   있다** — 백트래킹 깊이가 스택을 넘기면 `StackOverflowError` 로 **0.02초 만에 HTTP 500**
   이 된다(`(a|aa)+c` + 매치 실패하는 긴 줄). `Error` 라 일반 예외 처리로도 안 잡혀서
   매칭 지점에서 직접 삼킨다. ripgrep 은 유한 오토마타라 둘 다 원리적으로 없지만
   배포에 바이너리가 붙는다 — `DECISIONS.md` D12.
