...
- **가상 스레드를 껐다** — 켜져 있었지만 근거가 적힌 적이 없다. 아웃바운드 호출이 0이라
  기다릴 원격 대상이 없고(가상 스레드의 이득이 나오는 자리), 일이 CPU(Lucene)와 파일
  I/O(캐리어를 블록한다)이며, 동시성이 수십 규모다. **안전성이 JDK 버전에 걸려 있던
  것도 이유다** — `TrajectoryStore` 가 락을 쥔 채 파일 I/O 를 하므로 JDK 21 이었다면
  캐리어가 핀 됐고, JEP 491(JDK 24) 이후에야 무해해졌다. 근거와 되돌릴 조건은
  `application.yml` 주석에. **끄는 결정도 워크로드 분석이지 측정이 아니다.**
