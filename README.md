...
그것을 가능하게 하는 것이 **미러**입니다. 위키를 마크다운 파일로 통째로 받아둡니다.
...
학습 셋인데, 각자 값어치를 증명해야 하는 선택 사항입니다
...
| 말 | 뜻 |
|---|---|
| **볼트** | 미러가 놓인 디렉터리. 문서에서 미러와 거의 같은 뜻으로 씁니다 (기본값 `~/.wikilens`) |
| **코어** | 미러 + 위 연산 넷. 랭킹 층 없이도 도는 부분입니다 |
| **랭킹 층** | 코어 위에 얹는 선택 셋 — 앵커 · 형태소 검색 · 학습 |
| **앵커** | 다른 문서가 이 페이지를 링크할 때 쓴 표현. 이 도구의 핵심 신호입니다 |
| **궤적** | 누가 무엇을 검색해 무엇을 읽었는지의 기록. 서버판에서만 쌓입니다 |
