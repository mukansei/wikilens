...
2. [시작하기](#시작하기) — [로컬판 3줄](#로컬판--5분) · [서버판 사용자 2줄](#서버판--사용자) ·
...
| **팀이 세운 서버를 씁니다** | 볼트를 안 받습니다. 주소와 식별자만 | [서버판 사용자 — 2줄](#서버판--사용자) |
...
| 로컬판 CLI 설치(`pip install`) | **미검증** — 서버판은 이 분기를 안 지납니다 |
| 타 OS (Windows · 리눅스 데스크톱) | **미검증** — 이 머신은 macOS 하나입니다 |
...
- **로컬판의 `pip install`** — `setup` 이 `~/.wikilens/venv` 에 CLI 를 까는 경로입니다.
  서버판은 이미지가 CLI 를 날라 이 분기를 안 지납니다
- **Windows** — Git for Windows 없이(PowerShell 도구만) 볼트를 만드는 경로
- **리눅스 데스크톱** — 개발은 macOS 에서만 했습니다

<sub>**서버판 첫 구축은 빈 환경에서 밟았습니다**(2026-08-28) — `~/.wikilens`·이미지·
플러그인·마켓플레이스 클론을 전부 지운 뒤 `./server/wikilens-setup.sh` 하나로 볼트
13,990건·색인·기동까지. `venv` 가 안 생기는 것도 함께 확인했습니다.</sub>
