...
| 학습 | **발동 여부**(`served > 0`) | 층이 한 번도 말하지 않으면 켜둘 이유가 없습니다 |
| 학습 | 적중당 절약 대 미스당 검증 오버헤드 | 학습 레이어를 끄는 편이 낫습니다 |

> **`pWrong` 은 게이트가 아닙니다**(2026-08-28 재검토). 손익분기를 바르게 유도하면
> 서빙률이 소거되어 **`pWrong < (n−1)/n`** 이 되고, 기본 `limit=8` 에서 **0.875** 입니다 —
> 거의 항상 통과합니다. 한때 이 표가 "적중률보다 이것이 기준" 이라고 적었는데,
> 그 말은 **관측하기 쉽다**는 뜻이지 구속한다는 뜻이 아니었습니다.
> 진짜 관문은 **발동 여부**와 **비용 모델**(오답 시 `1+n` 읽기)이고, 후자는 아직
> 검증된 적이 없습니다 — [`docs/design/design-2026-08-28-1x-gate.md`](docs/design/design-2026-08-28-1x-gate.md).
