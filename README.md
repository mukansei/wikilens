...
**Confluence 를 에이전트가 읽을 수 있는 코퍼스로 만듭니다.**

에이전트는 위키를 못 읽습니다 — 인증이 필요하고, API 는 레이트리밋이 있고, 무엇보다
**grep 이 안 됩니다.** 그래서 "우리 배포 절차 문서 어디 있어?" 에 답할 수가 없습니다.

WikiLens 는 위키를 **디스크의 마크다운 파일**로 미러링합니다. 그 위에서 검색·읽기·
본문 스캔·계층 탐색이 전부 로컬 파일 연산이 됩니다:

```console
$ tree ~/.wikilens/vault
mirror/pages/01/200000001.md      ← 본문 (마크다운)
mirror/raw/01/200000001.xhtml     ← 원본 (재변환용)
ALIASES.md · TREE.md              ← 파생 색인
```

**그 위에 랭킹 층을 얹습니다** — 앵커 텍스트 색인, 한국어 형태소 검색, 팀의 탐색 이력
학습. 셋 다 **선택**이고 각자 값어치를 증명해야 합니다(→ [무엇이 실제로 값어치를
하나](#무엇이-실제로-값어치를-하나)). 코어는 그것 없이도 돕니다.
...
**무엇** — 위키를 디스크에 미러링해 **에이전트가 grep·read 할 수 있게** 만듭니다.
랭킹 층(앵커·형태소·학습)은 그 위의 선택입니다.
...
얹느냐**이고, 그래서 업그레이드 경로가 아니라 서로 다른 물건입니다.
...
형태소 분석 없이는 BM25 가 무너집니다 — 그것이 서버를 **JVM 으로 고른 유일한 이유**입니다.
...
|---|---|---|---|
| **코어** — 미러 + `read`·`grep`·`tree` | 위키를 로컬 파일로 만듭니다 | 코퍼스가 있으면 항상 | 13,926건 전부 |
...
**그 페이지들은 코어(제목 + 본문 grep)로만 발견됩니다.**
