...
| 결정 | 되돌리면 |
|---|---|
| **앵커 텍스트를 색인한다** | 가치의 대부분이 사라집니다 — 실데이터에서 순수 MD 검색 대비 정확도·토큰 둘 다 크게 앞섰습니다 (Brin & Page 1998, 신규 기법 아님) |
...
**전부 "낮으면 그만둔다" 로 읽습니다** — 높으면 좋다가 아니라, 낮으면 그 층이 값어치가 없다는 뜻입니다.

| 지표 | 무엇을 재나 | 낮으면 |
|---|---|---|
| 1. **제목과 다른 별칭 비율** | 어휘 격차가 실재하는가 | **여기서 중단** — 이 도구가 필요 없습니다 |
| 2. **비링크 도달 비율** | 학습이 링크 그래프 밖을 찾는가 | 학습 레이어가 링크 그래프의 **복사본으로 퇴화** |
| 3. **`pWrong`** | 서빙한 힌트 중 거부된 비율 | 손익분기 `p_hit > p_wrong/(n−1)` 의 분자 — **적중률보다 이것이 기준** |
| 4. 적중당 절약 대 미스당 검증 오버헤드 | 숏컷이 실제로 싼가 | 학습 레이어를 끄는 편이 낫습니다 |
