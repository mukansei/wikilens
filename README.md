...
| 디렉터리 | 내용 | 언어 | 상태 |
|---|---|---|---|
| **`cli/`** | Confluence 싱크 · 앵커 전치 · 로컬판 | Python | **55 테스트 통과** |
| **`server/`** | Lucene/Nori 색인 · 학습 레이어 | Kotlin | 핵심 32 통과 / 배선 미검증 |
| **`plugin/local/`** | 스킬만 (로컬판) | — | 형식 검증 |
| **`plugin/server/`** | MCP 도구 3개 + 스킬 (서버판) | Python | **18 테스트 통과** |
| `marketplace/` | 조직 배포용 | — | — |
...
| MCP 프록시 | 18개 테스트 통과 (핸드셰이크·도구·세션·404) |
| Kotlin 학습 레이어 (`learn/`) | 컴파일·실행 32/32, Python/scipy와 1e-6 일치 |
| Kotlin Lucene/Spring 배선 | **미검증** — 이 환경에서 Maven Central 접근 불가 |
