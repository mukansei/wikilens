...
`auto` 는 `rg` 가 PATH 에 있으면 씁니다. **JVM 스캔은 13,921건에 2.44초인데 grep 예산이
3초**라 약 17,000건에서 상시 잘림이 될 참이었습니다 — 실패가 아니라 **조용한 부분
응답**이라 사용자에게는 "없다"로 보입니다. rg 는 실측 4~6배라 그 한계를 약 70,000~
85,000건으로 밉니다.

> **D12 에 적힌 "65배" 는 이 맥락에서 틀린 수치입니다.** 그건 2,383건 시절 rg 를 셸에서
> 맨몸으로 돌린 값입니다. 서버 안에서는 ACL 대상 목록을 만드는 고정비를 두 엔진이
> 똑같이 내므로 4~6배입니다 — 스캔이 거의 없는 질의는 rg 가 **더 느립니다**(프로세스 기동).
...
왜 있느냐면, 지금의 "ACL" 이 실질적으로 **사용자 허용목록**이기 때문입니다 — `sync` 가
