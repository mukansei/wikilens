...
| `index/` `service/` `api/` `vault/` `acl/` (Lucene·Spring 배선) | 빌드·`bootRun`·재색인 검증됨 (Acme 실데이터 2,378건, 2026-08) |
...
- [정기 갱신·cron](#싱크는-python-그대로) · [분석기 고르기](#분석기는-색인-시점에-고른다) · [다국어 코퍼스](#다국어-코퍼스--판정은-build-가-한다) ·
...
저장소 코드(MIT)에는 안 옮지만, 이미지 재배포에는 그쪽 조건이 걸립니다.
직접 빌드해서 쓰는 지금 방식에는 해당 없습니다.
...
## 다국어 코퍼스 — 판정은 `build` 가 한다

**서버에는 설정이 없습니다.** `derived/excluded.json` 을 읽어 `wikilens build` 의
결정을 따릅니다.

```bash
wikilens build --root ~/.wikilens/vault --scripts hangul,ascii   # 그다음 재색인
```

같은 내용이 여러 언어로 있으면 읽지 못하는 언어의 문서가 어휘 순위에서 이길 수
있습니다. 실측(한·베 혼재 13,933건): 한국어 질의에 베트남어 번역본이 1위이고 한국어
원본이 10위 밖이었습니다.

**판정을 서버에 두지 않은 이유**는 로컬판이 아무것도 못 받기 때문입니다(그쪽은 서버가
없습니다). 그리고 `build` 는 이미 본문을 파싱하고 있어 계산이 공짜인데, 서버에 두면
같은 본문을 두 번 훑게 됩니다. **결정이 하나여야 두 판이 같은 문서 집합을 봅니다.**
...
> 않습니다.
>
> `derived/excluded.json` 이 **없으면 전부 색인합니다** — 이 기능 이전의 볼트가
> 그렇습니다. 못 읽어도 마찬가지입니다: 여기서 fail-closed 로 가면 파일 하나 깨진 것이
> 전 문서 소실이 됩니다.
...
(실측: 본문 `Acme` 에 `acme` 가 리터럴 1건 · 정규식 0건). rg 프로세스를 붙인다면
`-i` 를 함께 넘겨야 답이 같습니다.
