...
- [정기 갱신·cron](#싱크는-python-그대로) · [분석기 고르기](#분석기는-색인-시점에-고른다) · [다국어 코퍼스](#다국어-코퍼스--읽지-못하는-언어를-색인에서-뺀다) ·
...
## 다국어 코퍼스 — 읽지 못하는 언어를 색인에서 뺀다

```bash
--wikilens.index-scripts=hangul,ascii      # 기본은 빔 = 전부 색인
--wikilens.script-threshold=0.15
```

**왜 필요한가.** 같은 내용이 여러 언어로 있으면 **읽지 못하는 언어의 문서가 어휘
순위에서 이길 수 있습니다.** 실측(한·베 혼재 13,933건): 한국어 질의에 베트남어
번역본이 1위이고 한국어 원본이 10위 밖이었습니다.

두 문서가 같은 영문 식별자(`ga`·URL)를 공유하는데 번역본이 두 배 길어 tf 가 높았습니다.
앵커 층도 번역본을 올렸습니다(원본의 인링크 19개 중 18개가 `SDK 접속 가이드` 라고
불러서 `ga` 가 안 들어 있었습니다). **어휘 층은 정상 동작한 것이라 언어 말고는 가를
신호가 없습니다.**
...
이름이 없으면 범위를 직접 적습니다 — `"U+0100-017F"`.
...
**글자가 아니라 낱말로 셉니다.** 베트남어는 글자 대부분이 평범한 라틴이고 성조부호만
다릅니다(`Sử dụng` 에서 이질적인 글자는 2/7). 글자로 세면 신호가 묽어집니다 —
실측으로 같은 문서가 글자 4.17% 대 낱말 16.6% 였습니다.

**문턱은 코퍼스마다 다시 재세요.** 이 코퍼스에서는:

```
0.30 → 163건 제외 — 문제의 번역본이 안 걸린다(그 문서가 16.6%)
0.15 → 425건 제외 — 걸린다 (기본값)
0.10 → 519건
```

번역본은 원본과 섞여 있어서(영문 식별자·URL·코드가 그대로 남습니다) "전부 다른 언어"
를 기대하고 문턱을 높이면 정작 문제의 문서를 놓칩니다.

> **빠진 문서는 통째로 사라집니다** — `search`·`read`·`grep`·`tree` 넷 다 못 찾습니다.
> 넷이 전부 색인의 메타를 거치기 때문입니다. 그게 이 기능의 값어치이자 위험이라
> 제외 수를 기동 로그 · `/api/stats` 의 `droppedByScript` · `--status` 가 말합니다.
> **기본이 꺼짐인 것도 같은 이유입니다** — 켜짐이 기본이면 처음 띄운 사람의 문서가
> 조용히 사라집니다(ACL 과 빈 볼트로 두 번 물린 모양입니다).

**모르는 이름은 기동을 실패시킵니다.** 조용히 무시하면 운영자는 필터가 걸린 줄 알고
배포합니다 — 분석기 이름을 틀렸을 때와 같은 규칙입니다.
