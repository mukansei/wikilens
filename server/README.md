...
> **운영자용입니다.** 프로젝트 전체 안내는 [`../README.md`](../README.md) ·
> 서버판 사용자 안내는 [`../plugin/client/README.md`](../plugin/client/README.md) ·
> 볼트를 만드는 CLI 는 [`../cli/README.md`](../cli/README.md)

Confluence 미러를 **서버측에서 색인**하고, 탐색 궤적을 축적해 랭킹을 보강합니다.

**본문도 서버가 서빙합니다** — 클라이언트는 볼트를 갖지 않습니다. 이것이 이 판을
만든 이유입니다. 볼트를 나눠 주면 그 사본을 회수할 수 없어 권한을 못 거두는데,
서버가 서빙하면 **매 요청마다 권한을 봅니다**(`/api/read` 가 `markdown` 을 돌려줍니다).

<sub>여기 한때 "읽기는 클라이언트 로컬이고 서버는 좌표만 반환합니다" 라고 적혀
있었습니다(2026-08-27 정정). 초기 설계의 잔재였고 **이 문서의 두 번째 문장이
아키텍처와 반대**였습니다 — 실측하면 `read` 응답에 본문이 그대로 들어 있습니다.</sub>
...
것이면 [`docs/server-migration-and-rationale.md`](../docs/server-migration-and-rationale.md)
를 먼저 보세요 — **궤적은 유일한 복구 불가 자산**이라 놓치면 되돌릴 수 없습니다.
...
**옛 배포에서 올리기 · 왜 이렇게 만들었나** — 세울 때는 안 읽는 둘입니다
- [`../docs/server-migration-and-rationale.md`](../docs/server-migration-and-rationale.md)
  — 궤적 옮기기(**복구 불가 자산**) · Docker 볼륨 · 설계 근거 여섯
...
**설정은 "무엇으로 색인할까"이지 "무엇으로 질의할까"가 아닙니다.** 질의는 항상 디스크
색인이 실제로 만들어진 분석기를 씁니다 — 선택이 Lucene 커밋 데이터에 기록되고(색인과
...
```
색인은 'korean' 로 색인했고 설정은 'english' 입니다…     ← 순간적으로 찍힌다
...
# 옛 배포에서 올리기 · 왜 이렇게 만들었나

**옛 배포에서 올린다면 궤적을 먼저 옮기세요** — `state/trajectories.jsonl` 은
**유일한 복구 불가 자산**입니다. 색인은 재구축되지만 이것은 안 됩니다.

설계 근거(왜 서버가 색인을 갖는가 · 왜 JVM 인가 · 볼트를 클라이언트에 안 주는 이유 ·
훅이 없는 이유 · 토크나이저 정본)도 함께 옮겼습니다. **되돌리기 전에 읽을 것.**
