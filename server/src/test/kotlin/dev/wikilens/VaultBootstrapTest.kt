...
class VaultBootstrapTest {
    ...
    fun `버려진 세션을 스케줄러가 거둔다`() {
        val sunk = ArrayList<dev.wikilens.learn.Trajectory>()
        val store = dev.wikilens.learn.TrajectoryStore(sink = { sunk.add(it) })
        ...
        SessionSweeper(store, WikiLensProperties(learn = dev.wikilens.config.LearnProps(sessionIdleMillis = -1))).sweep()
