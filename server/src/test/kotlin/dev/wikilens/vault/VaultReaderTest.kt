...
/**
 * `contracts/fixtures/mini-vault/`는 Python `cli/tests/test_contract_fixtures.py`와
 * **공유하는 정본 볼트**다. Python이 `build()`로 이 픽스처를 재생성해 형식을 검증하고,
 * 여기서는 같은 체크인된 산출물을 `VaultReader`가 그대로 소화하는지 확인한다.
 *
 * grep 기반 계약 검사(`check-contracts.sh`)는 문자열이 존재하는지만 보므로 리팩터링에
...
class VaultReaderTest {
    ...
    private val fixtureRoot: Path = Path.of("..", "contracts", "fixtures", "mini-vault")
