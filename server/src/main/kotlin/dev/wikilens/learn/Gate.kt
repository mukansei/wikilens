...
object Gate {
    ...
    private val TRACING = listOf(
        // `흐르다` 는 ㄹ 불규칙이라 실제로 치는 말은 "흘러" 다 — 어간형만 있어서
        // "배포가 어떻게 흘러가나" 가 TRACING 을 놓치고 길이 폴백으로 LOCALIZATION 이
        // 됐다(실측). 경로 의존 질의가 간선을 만드는 쪽으로 새는 것이라 방향이 나쁘다.
        "흐름", "어떻게 동작", "어떻게 흐르", "어떻게 흘러", "어떻게 처리",
        "호출 경로", "호출 체인",
        "거쳐", "생명주기", "단계별로",
