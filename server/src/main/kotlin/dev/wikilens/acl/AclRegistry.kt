 ...
 *
 * 끄면 등록 없이 전원이 전 문서를 본다. 정당한 경우는 좁다 — 볼트를 싱크한 계정의 권한
 * 범위를 이 서버에 닿는 전원이 공유해도 되는 배포뿐이다. 기본값은 **켜짐**: 조용히
 * 열리는 것보다 조용히 빈손인 편이 낫다(후자는 눈에 띈다). 꺼져 있으면 기동 로그와
 * `/api/stats`·`--status` 가 말한다. 왜 이 스위치가 필요했는지는 `DECISIONS.md`.
 */
class AclRegistry(
