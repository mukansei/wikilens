...
/**
 * 본문 스캔 엔진 — JVM 내장 스캔과 ripgrep 프로세스.
 *
 * **ACL 은 여기 없다.** 호출부([ContentService])가 이미 거른 목록을 넘긴다 — 권한 해석이
 * 엔진마다 갈리면 한쪽이 조용히 더 보여준다.
 *
 * 경로가 둘인 것은 피할 수 없다(rg 가 없는 머신이 있다). **두 구현이 같은 답을 내는지는
 * 이 인터페이스가 아니라 `GrepEngineParityTest` 가 지킨다.**
 */
