...
class FileTrajectorySink(stateDir: Path, private val mapper: ObjectMapper) : TrajectorySink {
    ...
    override fun append(t: Trajectory) {
        runCatching {
            ...
            // 앞서가고, WARN 한 줄로는 재기동 때까지 아무도 모른다.
    ...
    fun replayInto(store: TrajectoryStore): Int {
        if (!Files.exists(file)) {
            // **조용히 0을 돌려주면 안 된다.** 상태 디렉터리 기본값이 상대경로라 작업
            // 디렉터리가 달라지면 여기가 다른 자리를 가리키고, 옛 궤적을 못 읽은 채
        ...
        Files.newBufferedReader(file).useLines { lines ->
            for (line in lines) {
                ...
                runCatching { store.replay(mapper.readValue<Trajectory>(line)) }
                    .onSuccess { n++ }
                    .onFailure {
                        // **삼키면 안 된다.** 필수 필드를 하나 더하는 순간 옛 줄이 전부
                        // 실패하는데, 조용하면 로그가 `궤적 0건 재생` 이라 **첫 배포와
                        // 구별되지 않는다.** 스키마는 실제로 두 번 바뀌었다.
                        bad++
        ...
        // append-only 라 **줄지 않는다.** 아직 문제는 아니지만(측정: 100만 건이 210MB ·
        // 재생 5.3초 · 힙 80MB) 아무도 안 보면 기동이 조용히 느려진다. 압축 대신
        // **보이게** 한 근거는 `DECISIONS.md` D17.
    ...
    /** `/api/stats` 로 나가는 로그 진단. 파일 싱크 고유의 일이라 **한 자리에 모은다.** */
