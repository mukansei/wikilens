...
ensure_admin_token() {
  ...
  # **`-r` 이 아니라 `-s` 다 — 0바이트도 "읽을 수 있다".** 디스크가 차거나 쓰다 죽으면
  # 빈 파일이 남는데, 그러면 빈 토큰을 쥐고 "읽었습니다" 를 찍는다. 서버는 값이 비면
  # 404 라 동작은 안전한 쪽으로 닫히지만, **운영자가 토큰을 의심하지 않게 된다.**
  # 비어 있으면 아래로 떨어져 새로 만든다(덮어쓰기가 맞다 — 쓸모없는 값이다).
  if [ -s "$f" ]; then
  ...
  [ -e "$f" ] && echo "관리 토큰 파일이 비어 있습니다 — 새로 만듭니다: $f"
